...
// Public self-signup is OFF by default in real environments (see
// .env/.env.local + src/app/api/auth/signup/route.ts) now that accounts are
// admin-created — but the existing signup route tests exercise the real
// signup logic and shouldn't need touching just because of that unrelated
// flag, so tests default it to enabled.
