...
export async function api<T = unknown>(path: string, options: ApiOptions = {}): Promise<T> {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          try {
            ...
            // `message` is the human-readable string meant for display;
            // `error` is a stable machine code meant for `if (err.code === …)`
            // branching (see ApiErrorCode above) — using it as the fallback
            // display text previously leaked raw codes like "ALREADY_PAID"
            // straight into the UI whenever a caller didn't special-case it.
      ...
      // A 204/205 (or any other empty-body success, e.g. DELETE) has nothing
      // to parse — response.json() throws on an empty body, which previously
      // surfaced as a false "network error" even though the request succeeded.
      const text = await response.text();
