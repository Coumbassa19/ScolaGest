...
export function createBictorysProvider(env: BictorysEnv): BictorysProviderHandle {
  ...
  async function charge(input: ChargeInput): Promise<ChargeResult> {
    ...
    // ISO 3166-1 alpha-2 country. Defaults to 'SN' (Senegal) — this
    // provider's original, verified-working market — for every existing
    // caller. A caller targeting a different Bictorys-supported market
    // passes `metadata.country` (e.g. 'GN' for Guinea) to override.
    // UNVERIFIED: whether Bictorys actually supports Guinea/GNF at all has
    // not been confirmed as of this writing — check with Bictorys before
    // relying on a non-'SN' country in production.
    const country =
      typeof input.metadata?.country === 'string' ? (input.metadata.country as string) : 'SN';
