...
// getRequestLocale() reads next/headers cookies() — mock it so the route
// doesn't throw "cookies() called outside a request scope" in tests.
// Absent a cookie, it falls back to DEFAULT_LOCALE ('fr').
mockNextCookies();
...
describe('POST /api/auth/signup', () => {
  it('creates a new user, code, and outbox event for genuinely new emails', async () => {
    ...
    const res = await POST(makeReq({ email: 'new@example.com', password: 'A-strong-passphrase-1' }));
  ...
  it('returns identical 201 + dummy-bcrypts on existing email (enumeration-resist)', async () => {
    ...
    const res = await POST(
      makeReq({ email: 'existing@example.com', password: 'A-strong-passphrase-1' }),
    );
    expect(res.status).toBe(201);
    const body = await res.json();
    expect(body).toEqual({ ok: true });
  ...
  it('returns VALIDATION_FAILED for malformed email', async () => {
    const res = await POST(makeReq({ email: 'not-an-email', password: 'A-strong-passphrase-1' }));
    expect(res.status).toBe(400);
    const body = await res.json();
