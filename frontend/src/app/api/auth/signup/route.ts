...
//
// PUBLIC_SIGNUP_ENABLED gate (Phase 5 of the RBAC rollout — see
// scripts/backfill-teacher-assignments.ts's sibling work): now that staff
// accounts are admin-created with a secure setup link (POST
// /api/admin/users), public self-registration is OFF by default — a plain
// 404 that doesn't even confirm the route exists, same posture as a
// removed endpoint. Flip PUBLIC_SIGNUP_ENABLED=1 to re-enable (e.g. if a
// future non-staff public registration flow reuses this route — read that
// decision fresh rather than assuming this comment still applies).
...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  return withRequestContext(ctx, async () => {
    if (process.env.PUBLIC_SIGNUP_ENABLED !== '1') {
