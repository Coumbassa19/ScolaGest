...
const subscriptionPaymentFindFirst = vi.fn();
const subscriptionPaymentUpdate = vi.fn();
...
const $transaction = vi.fn(async (fn: (tx: unknown) => Promise<unknown>, _opts?: unknown) =>
  fn({
    ...
    schoolSubscriptionPayment: { findFirst: subscriptionPaymentFindFirst, update: subscriptionPaymentUpdate },
...
beforeEach(() => {
  ...
  subscriptionPaymentFindFirst.mockReset();
  subscriptionPaymentUpdate.mockReset();
...
describe('POST /api/webhooks/bictorys', () => {
  ...
  it('onPaid resolves a SchoolSubscriptionPayment (not an Order) and extends the school period', async () => {
    findUnique.mockResolvedValueOnce(null);
    orderFindFirst.mockResolvedValueOnce(null); // no matching Order
    subscriptionPaymentFindFirst.mockResolvedValueOnce({
      id: 'sp1',
      schoolId: 'school-1',
    });
    schoolFindUnique.mockResolvedValueOnce({ currentPeriodEnd: null });
    const { POST } = await import('./route');
    const { req } = bictorysFixtureRequest({ status: 'succeeded' });
    const res = await POST(req);
    expect(res.status).toBe(200);
    expect(subscriptionPaymentUpdate).toHaveBeenCalledWith(
