...
export default async function ImportStudentsPage() {
  ...
  const t = await getTranslations('students.importPage');
    ...
    <div className="flex flex-col md:flex-row bg-background min-h-full font-body">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 px-4 py-4 md:px-8 md:py-6">
          <ImportStudentsForm classes={classes} academicYears={academicYears.map((y) => y.label)} />
        </div>
      </div>
    </div>
  );
